...
def get_angle(v1: List[float],
    ...
    """
    if len(v1) != len(v2):
        raise ValueError(f'v1 and v2 must be the same length, got {len(v1)} and {len(v2)}.')
...
def get_dihedral(v1: List[float],
    ...
    if len(v1) != 3 or len(v2) != 3 or len(v3) != 3:
        raise ValueError(f'v1, v2, and v3 must have a length of three, got {len(v1)}, {len(v2)}, and {len(v3)}.')
    ...
    if np.isnan(dihedral):
        raise ValueError(f'Could not calculate a dihedral angle for\nv1: {v1}, v2: {v2}, v3: {v3}.')
    conversion = 180 / math.pi if 'degs' in units else 1
...
def calculate_distance(coords: Union[list, tuple, dict],
    ...
    if index not in [0, 1]:
        raise ValueError(f'index must be either 0 or 1, got {index}')
    if len(atoms) != 2:
        raise ValueError(f'distance atom list must be of length two, got {len(atoms)}')
    if len(set(atoms)) < 2:
        raise ValueError(f'some atoms are repetitive: {atoms}')
    new_atoms = list()
    for atom in atoms:
    ...
    if not all([isinstance(a, int) for a in new_atoms]):
        raise ValueError(f'all entries in atoms must be integers, got: {new_atoms} ({[type(a) for a in new_atoms]})')
...
def calculate_angle(coords: Union[list, tuple, dict],
    ...
    if index not in [0, 1]:
        raise ValueError(f'index must be either 0 or 1, got {index}')
    if len(atoms) != 3:
        raise ValueError(f'angle atom list must be of length three, got {len(atoms)}')
    if len(set(atoms)) < 3:
        raise ValueError(f'some atoms are repetitive: {atoms}')
    new_atoms = list()
    for atom in atoms:
...
def calculate_dihedral_angle(coords: Union[list, tuple, dict],
    ...
    if index not in [0, 1]:
        raise ValueError(f'index must be either 0 or 1, got {index}')
    if torsion is None:
        raise ValueError(f'torsion cannot be None')
    if len(torsion) != 4:
        raise ValueError(f'torsion atom list must be of length four, got {len(torsion)}')
    if len(set(torsion)) < 4:
        raise ValueError(f'some indices in torsion are repetitive: {torsion}')
    new_torsion = list()
    for atom in torsion:
        if isinstance(atom, str) and 'X' in atom:
    ...
    if not all([isinstance(t, int) for t in new_torsion]):
        raise ValueError(f'all entries in torsion must be integers, got: {new_torsion} '
